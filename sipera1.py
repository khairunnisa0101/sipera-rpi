...
try:
    while True:
        ...
        frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)  # perbaiki urutan warna
            ...
            try:
                ...
                if len(result) > 0 and len(result[0]) > 0:
                    ...
                    if presensi_key not in presensi_harian:
                        ...
                        # --- Cetak nilai atribut di terminal ---
                        ...
                        for nama, conf in atribut.items():
                            if nama == "sabuk":
                                if conf < 0.3:  # threshold lebih rendah untuk sabuk
                                    lengkap = "0"
                                    break
                            else:
                                if conf < 0.4:  # threshold normal untuk atribut lain
                                    lengkap = "0"
                                    break
                        ...
                        try:
                            ...
                            # Simpan traceback lengkap ke file log
                            with open("error_log.txt", "a") as log:
                                log.write(f"\n[{datetime.datetime.now()}] Exception saat kirim data API:\n")
                                log.write(traceback.format_exc())  # simpan seluruh pesan error lengkap

                            # Tampilkan versi singkat di terminal
